...
async function valyuSearch(
    ...
    if (!res.ok) {
        const text = await res.text();
        throw new Error(`Valyu API error: ${text}`);
    }
...
/**
 * GET /api/company-comparison?base_company=Shell
 */
export async function GET(req: NextRequest) {
    try {
        const { searchParams } = new URL(req.url);
        const baseCompany = searchParams.get("base_company");
        ...
        const rows = await runCompanyComparison(baseCompany);
